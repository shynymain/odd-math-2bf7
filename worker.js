export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (!["entry","entryRow","result"].includes(mode)) return json({ok:false,error:"unknown endpoint", endpoint:mode});
      ...
      return json({ok: !!parsed.ok || !!parsed.race || !!parsed.horse || !!parsed.result, mode, rawText: payload, parsed});
    } catch (e) {
...
function buildPrompt(mode){
  if (mode === "entry") return `あなたは競馬出馬表ヘッダー専用OCRです。必ずJSONだけ。説明禁止。推測禁止。抽出対象：年月日、開催地、レース番号、レース名、馬場、距離、対象年齢、対象性別、条件、グレード、頭数。馬名・結果・払戻は出力しない。形式：{"ok":true,"race":{"date":"","place":"","raceNo":"","raceName":"","grade":"","condition":"","age":"","sex":"","surface":"","distance":"","headcount":""}}`;
...
function parseBestJson(text){
  ...
  for (let i=0;i<s.length;i++){
    ...
    if (s[i] === "}") { depth--; if (depth === 0 && start >= 0) { try { out.push(JSON.parse(s.slice(start, i+1))); } catch(e){} } }
  }
