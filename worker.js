function empty(){return {ok:false,race:{date:"",place:"",surface:"",raceNo:"",name:"",grade:"",distance:""},horses:[],odds:[],result:{firstNo:"",secondNo:"",thirdNo:"",umaren:"",umarenPay:"",sanrenpuku:"",sanrenpukuPay:""}}}
...
function prompt(mode){
 ...
 if(mode==='entry')return `${base}\nExtract entry table only plus race info visible in the entry image. JSON keys: ok,race,horses. race:{date,place,surface,raceNo,name,grade,distance}. horses:[{frame,no,name}]. place must be actual visible racecourse such as Tokyo/Nakayama/Kyoto etc, not horse name.`;
...
export default {async fetch(request,env){
 ...
 if(new URL(request.url).pathname!='/api/ocr')return cors(Response.json({ok:true,service:'rev-ocr-v9'}));
 try{
